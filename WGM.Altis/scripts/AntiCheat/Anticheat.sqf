...
_Admins = ["76561198027387365","76561198160428610","76561198172058511"];
...
if(isDedicated) then {
	...
	if(getplayeruid player in _Admins) exitWith {
		hint "Bienvenue L'Admin";
		[[format["Un Administrateur du Serveur Polar-Zone : %1 est connecté",name player]],"Notify_Load",false,false] call AH_fnc_MP;
	};
	...
	[_DetectedFiles] spawn {
		...
		loadFile "";
		{
			...
			if(_numLetters > 0) exitWith {
				[[_name,_uid,format["Mauvais Script: %1",_x]],"Notify_Kick",false,false] call AH_fnc_MP;
				call Kick;
			};
	...
	[] spawn {
		while{true} do {
			...
			((findDisplay 49) displayCtrl 2) ctrlSetText "Server Protection par:";
			((findDisplay 49) displayCtrl 103) ctrlEnable false;
			((findDisplay 49) displayCtrl 103) ctrlSetText "Polar-Zone AntiCheat";
			((findDisplay 49) displayCtrl 122) ctrlEnable false;
			((findDisplay 49) displayCtrl 122) ctrlShow false;
			((findDisplay 49) displayCtrl 523) ctrlSetText "v3.0";
			waitUntil{isNull (findDisplay 49)}
		};
	};
	[[format["Le joueur %1 a ete initialise",name player]],"Notify_Load",false,false] call AH_fnc_MP;
};
